...
/* F16C converts binary16 to float32; AVX alone does not provide it.
 * SQLITE_VEC_ENABLE_AVX already selects x86. Function target attributes need
 * GCC or Clang; other compilers keep the scalar implementation. */
...
static int vec_half_has_f16c(void) {
#ifdef _MSC_VER
  /* clang targeting the MSVC runtime has no libgcc __cpu_model. */
  int cpu[4];
  __cpuid(cpu, 1);
  const int required = (1 << 27) | (1 << 28) | (1 << 29);
  return (cpu[2] & required) == required && (_xgetbv(0) & 6) == 6;
#else
  /* Older compilers' F16C check does not include AVX OS support. */
  return __builtin_cpu_supports("avx") && __builtin_cpu_supports("f16c");
#endif
}

__attribute__((target("avx,f16c")))
static f32 l2_sqr_float16_f16c(const void *a, const f32 *q, size_t n) {
  ...
  for (; i + 32 <= n; i += 32) {
    for (int lane = 0; lane < 4; lane++) {
      ...
      sums[lane] = _mm256_add_ps(sums[lane], _mm256_mul_ps(diff, diff));
    }
  }
  for (; i + 8 <= n; i += 8) {
  ...
  f32 s[8];
  _mm256_storeu_ps(s, _mm256_add_ps(_mm256_add_ps(sums[0], sums[1]),
                                  _mm256_add_ps(sums[2], sums[3])));
  f32 sum = 0;
  for (int j = 0; j < 8; j++) sum += s[j];
  for (; i < n; i++) {
    f32 diff = vec_half_at(a, i) - q[i];
    sum += diff * diff;
  }
  return sqrtf(sum);
}
...
__attribute__((target("avx,f16c")))
static f32 l1_float16_f16c(const void *a, const f32 *q, size_t n) {
  __m256 sums[4] = {_mm256_setzero_ps(), _mm256_setzero_ps(),
                    _mm256_setzero_ps(), _mm256_setzero_ps()};
  ...
  for (; i + 32 <= n; i += 32) {
    for (int lane = 0; lane < 4; lane++) {
      ...
      __m256 y = _mm256_loadu_ps(q + i + lane * 8);
      __m256 diff = _mm256_sub_ps(x, y);
      sums[lane] = _mm256_add_ps(sums[lane], _mm256_andnot_ps(sign_mask, diff));
    }
  }
  for (; i + 8 <= n; i += 8) {
    __m256 x = _mm256_cvtph_ps(_mm_loadu_si128((const __m128i *)((const u8 *)a + i * 2)));
    __m256 y = _mm256_loadu_ps(q + i);
    __m256 diff = _mm256_sub_ps(x, y);
    sums[0] = _mm256_add_ps(sums[0], _mm256_andnot_ps(sign_mask, diff));
  }
  f32 s[8];
  _mm256_storeu_ps(s, _mm256_add_ps(_mm256_add_ps(sums[0], sums[1]),
                                  _mm256_add_ps(sums[2], sums[3])));
...
__attribute__((target("avx,f16c")))
static f32 cosine_float16_f16c(const void *a, const f32 *q, size_t n, f32 query_norm) {
  __m256 sums[4] = {_mm256_setzero_ps(), _mm256_setzero_ps(),
                    _mm256_setzero_ps(), _mm256_setzero_ps()};
  ...
  for (int j = 0; j < 8; j++) sum += s[j];
  f32 m[8];
  _mm256_storeu_ps(m, _mm256_add_ps(_mm256_add_ps(norms[0], norms[1]),
                                  _mm256_add_ps(norms[2], norms[3])));
  f32 norm = 0;
  for (int j = 0; j < 8; j++) norm += m[j];
  for (; i < n; i++) {
...
/* The query has already been widened once by the caller. Like the existing
 * distance_l2_sqr_float/int8 functions, L2 returns the Euclidean distance. */
static f32 l2_sqr_float16(const void *a, const f32 *q, size_t n) {
  f32 sum = 0;
...
static f32 distance_l2_sqr_float16(const void *a, const f32 *q, size_t n) {
#ifdef VEC_HAVE_F16C
  if (vec_half_has_f16c()) {
    return l2_sqr_float16_f16c(a, q, n);
  }
#endif
  return l2_sqr_float16(a, q, n);
}

static f32 l1_float16(const void *a, const f32 *q, size_t n) {
  f32 sum = 0;
  for (size_t i = 0; i < n; i++) {
    f32 diff = vec_half_at(a, i) - q[i];
    sum += fabsf(diff);
...
static f32 distance_l1_float16(const void *a, const f32 *q, size_t n) {
#ifdef VEC_HAVE_F16C
  if (vec_half_has_f16c()) {
    return l1_float16_f16c(a, q, n);
  }
#endif
  return l1_float16(a, q, n);
}

static f32 cosine_float16(const void *a, const f32 *q, size_t n, f32 query_norm) {
  f32 sum = 0;
  f32 norm = 0;
  for (size_t i = 0; i < n; i++) {
    f32 x = vec_half_at(a, i);
...
static f32 distance_cosine_float16(const void *a, const f32 *q, size_t n, f32 query_norm) {
#ifdef VEC_HAVE_F16C
  if (vec_half_has_f16c()) {
    return cosine_float16_f16c(a, q, n, query_norm);
  }
#endif
  return cosine_float16(a, q, n, query_norm);
}
